...
const { PassThrough: BuiltInPassThrough } = require('stream');
...
class XLSX {
  ...
  async addWorksheets(zip, model) {
    ...
    function streamXmlData(xmlStream, zip, chunkSize = 1000, worksheetId) {
        console.log("Streaming XML data in chunks...");

        // Create a PassThrough stream
        const passThroughStream = new BuiltInPassThrough();

        // Append the PassThrough stream to the zip as a file
        zip.append(passThroughStream, { name: `xl/worksheets/sheet${worksheetId}.xml` });

        // Write data to the PassThrough stream in chunks
        for (let i = 0; i < xmlStream._xml.length; i += chunkSize) {
            const chunk = xmlStream._xml.slice(i, i + chunkSize);
            if (chunk && chunk.length > 0) {
                // Join the chunk and write it to the PassThrough stream
                const chunkString = chunk.join('');
                passThroughStream.write(chunkString);
                console.log(`Writing chunk ${i / chunkSize + 1}`);
            }
        }

        // End the PassThrough stream to signal that the data is complete
        passThroughStream.end();
        console.log("Finished streaming XML data.");
    }
    ...
    model.worksheets.forEach(worksheet => {
        ...
        // Use the streamXmlData function to stream XML data to the ZIP
        streamXmlData(xmlStream, zip, 1000, worksheet.id);
